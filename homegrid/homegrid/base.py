...
class Storage(WorldObj):
    ...
    def __init__(self, name, textures, state=None, action=None,
        contains=None, reset_broken_after=20, seed=None):
        ...
        # update 11.22: remove "open" state if not initiated with a fixed state
        self.state = state if state else \
            random.choice(["closed"])
    ...
    def interact(self, action, obj=None):
        if action == "get":
          if self.state == "broken":
            return False, "Cannot get object from a broken Storage object."
          elif self.state != "open":
            return False, "Cannot get object from a closed Storage object."
          else:
            return self._get_contents(), None
        elif action == "drop":
          if self.state == "broken":
            return False, "Cannot get object from a broken Storage object."
          elif self.state != "open":
            return False, "Cannot drop object to a closed Storage object."
          elif len(self.contains) == 0 and obj:
            obj.cur_pos = (-1, -1)
            self.contains.append(obj)
            return True, None
          else:
            return False, "Cannot drop to a Storage object when it contains another object inside."
          ...
          if self.state == "closed":
            if action != self.action:
              ...
              return False, f"Cannot {action} a broken Storage object."
          else:
              return False, f"Fail to open a storage object which is already open."
        else:
...
class MiniGridEnv(gym.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self._gen_grid(self.width, self.height, seed=seed)
