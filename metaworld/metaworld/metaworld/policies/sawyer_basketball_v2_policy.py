...
class SawyerBasketballV2Policy(Policy):
    ...
    def _desired_pos(o_d: dict[str, npt.NDArray[np.float64]]) -> npt.NDArray[Any]:
        ...
        if np.linalg.norm(pos_curr[:2] - pos_ball[:2]) > 0.04 or (abs(pos_curr[2] - pos_ball[2]) > 0.025 and o_d["gripper"]<0.3):
            return pos_ball + np.array([0.0, 0.0, 0.3]),"Place gripper above the ball."
        elif abs(pos_curr[2] - pos_ball[2]) > 0.025:
            return pos_ball, "Grasp the ball."
        elif abs(pos_ball[2] - pos_hoop[2]) > 0.025:
            return np.array([pos_curr[0], pos_curr[1], pos_hoop[2]]), "Aim at the target."
        else:
            return pos_hoop, "Get to the target."
