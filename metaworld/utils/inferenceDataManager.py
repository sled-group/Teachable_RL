...
class inferenceDataManager():
    def __init__(self,state_mean,state_std,empty_language_encode, segment_length=6):
    ...
    def prepareTrajectory(self, trajectory,env_name="messenger"):
            ...
            return {
                'text_input': text.unsqueeze(0).unsqueeze(0).to("cuda").float(),
                'states': states.unsqueeze(0).to("cuda").float(),
                'actions': actions.float(),
                'returns_to_go': return_to_go.float(),
                'languages': language.to("cuda").unsqueeze(0).float(),
